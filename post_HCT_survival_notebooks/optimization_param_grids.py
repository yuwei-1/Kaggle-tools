    ...
    def get(trial: optuna.Trial):
        params = {
            ...
            "min_child_weight": trial.suggest_float(
                "min_child_weight", 1e-4, 100, log=True
            ),
...
class LGBMGBDTParamGrid(BaseLGBMParamGrid):
    def get(self, trial: optuna.Trial):
        ...
        params = {"boosting_type": "gbdt", "data_sample_strategy": "bagging"}
...
class LGBMGBDTGossParamGrid(BaseLGBMParamGrid):
    def get(self, trial: optuna.Trial):
        base_params = super().get(trial)
        params = {"boosting_type": "gbdt", "data_sample_strategy": "goss"}
        params.update(base_params)
        return params
...
class LGBMRFParamGrid(BaseLGBMParamGrid):
    def get(self, trial: optuna.Trial):
        base_params = super().get(trial)
        params = {"boosting_type": "rf", "num_boost_round": 10000}
        params.update(base_params)
        return params
...
class BaseXGBoostParamGrid:
    ...
    def get(trial: optuna.Trial):
        params = {
            ...
            "min_child_weight": trial.suggest_float(
                "min_child_weight", 0.1, 100, log=True
            ),
            ...
            "max_cat_threshold": trial.suggest_int(
                "max_cat_threshold", 1, 1000, log=True
            ),
            "max_cat_to_onehot": trial.suggest_int(
                "max_cat_to_onehot", 1, 1000, log=True
            ),
            # "scale_pos_weight" : trial.suggest_float("scale_pos_weight", 1, 1000, log=True),
        }
        return params
...
class XGBoostGBTreeLossguide(BaseXGBoostParamGrid):
    def get(self, trial: optuna.Trial):
        base_params = super().get(trial)
        params = {"booster": "gbtree", "grow_policy": "lossguide"}
        params.update(base_params)
        return params
...
class XGBoostGBTreeDepthwise(BaseXGBoostParamGrid):
    def get(self, trial: optuna.Trial):
        base_params = super().get(trial)
        params = {"booster": "gbtree", "grow_policy": "depthwise"}
        params.update(base_params)
        return params
    ...
    def get(trial: optuna.Trial):
        params = {
            ...
            "bagging_temperature": trial.suggest_float(
                "bagging_temperature", 0.1, 100, log=True
            ),
            "subsample": trial.suggest_float("subsample", 0.7, 1.0),
            "colsample_bylevel": trial.suggest_float("colsample_bylevel", 0.7, 1.0),
            "min_data_in_leaf": trial.suggest_float(
                "min_data_in_leaf", 1, 1000, log=True
            ),
...
class YDFParamGrid:
    ...
    def get(trial: optuna.Trial):
        params = {
            ...
            "categorical_set_split_min_item_frequency": trial.suggest_int(
                "categorical_set_split_min_item_frequency", 1, 200
            ),
            ...
            "l1_regularization": trial.suggest_float(
                "l1_regularization", 1e-4, 1e2, log=True
            ),
            "l2_categorical_regularization": trial.suggest_float(
                "l2_categorical_regularization", 1e-4, 1e2, log=True
            ),
            "l2_regularization": trial.suggest_float(
                "l2_regularization", 1e-4, 1e2, log=True
            ),
