...
class SHAPFeatureSelector:
    def __init__(
        ...
        feature_importance_save_path: Union[str, None] = None,
    ) -> None:
    ...
    def select(self, X: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self._plot_shap_values(
            sorted_features[: self._top_k_features],
            sorted_shap_values[: self._top_k_features],
        )
        return sorted_features, sorted_shap_values

    def _plot_shap_values(
        self, top_features: np.ndarray, top_importances: np.ndarray
    ) -> None:
        ...
        plt.barh(
            top_features, top_importances, color=colors, edgecolor="black", height=0.5
        )
