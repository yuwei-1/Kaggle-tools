...
class BasicDatasetEDA:
    ...
    def review(self):
        ...
        print("="*50, title, "="*50)
        print("Train: ", (100*self._train_df.isna().sum() / self._train_df.shape[0]).to_dict())
        print("Test: ", (100*self._test_df.isna().sum() / self._test_df.shape[0]).to_dict())
        ...
        plt.boxplot(combined_df[self._target_col_name], vert=True, patch_artist=True)  # vert=True for vertical boxplot
    ...
    def _plot_categorical(self, categorical_columns):
        ...
        for i, col in enumerate(categorical_columns):
            ...
            axes[i].barh([val if len(val) < 7 else val[:4] + '...' for val in train_counts.keys()], list(train_counts.values()), label="Train dataset")
            axes[i].barh([val if len(val) < 7 else val[:4] + '...' for val in test_counts.keys()], list(test_counts.values()), label="Test dataset")
    ...
    def _plot_numerical(self, numerical_columns):
        ...
        for i, col in enumerate(numerical_columns):
            axes[i].scatter(self._train_df[col], self._train_df[self._target_col_name], alpha=0.5, color="blue")
