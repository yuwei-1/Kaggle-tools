...
class RecursiveFeatureEliminationExecutor:
    ...
    def run(self, X, y):
        ...
        if self._verbose:
            print("#" * 15 + " Running CV with all features " + "#" * 15)
            print(f"Score achieved: {best_score}")

        is_minimize = self._direction == "minimize"
        while (len(self._initial_features) > self._minimum_num_features) and (
            running_scores[-1] <= best_score
        ) * is_minimize:
            ...
            for i, feat in enumerate(self._initial_features):
                ...
                if self._verbose:
                    ...
                    print(
                        "#" * 15
                        + f" Running CV without feature: {self._initial_features[i]} "
                        + "#" * 15
                    )

                score_tuple, oof, _ = self._cross_validation_executor.run(
                    X[used_features], y
                )
