...
class PCAFeatureCreator:
    ...
    def _condense_features(self, train_features : pd.DataFrame, test_features : pd.DataFrame):
        pca_train = self.pca_model.fit_transform(train_features)
        pca_test = self.pca_model.transform(test_features)
        mask = self.pca_model.explained_variance_ratio_.cumsum() <= self._variance_threshold
    ...
    def create(self, train_data : pd.DataFrame, test_data : pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        pca_train, pca_test, num_components = self._condense_features(train_features_to_reduce, test_features_to_reduce)

        pca_train = pd.DataFrame(index=train_data.index, 
                                 data=pca_train, 
                                 columns=[f"PCA component {i}" for i in range(pca_train.shape[1])])
        pca_test = pd.DataFrame(index=test_data.index, 
                                data=pca_test, 
                                columns=[f"PCA component {i}" for i in range(pca_train.shape[1])])
