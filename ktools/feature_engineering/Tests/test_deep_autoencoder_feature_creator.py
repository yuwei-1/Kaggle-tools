...
class TestDeepFeatureCreator(unittest.TestCase):
    def test_create_deep_features(self):
        ...
        X, _ = make_regression(
            n_samples=1000, n_features=num_features, noise=0.1, random_state=42
        )
        ...
        # Act
        creator = DeepFeatureCreator(
            train_data, all_feature_names, levels_of_compression, logger=logger
        )
