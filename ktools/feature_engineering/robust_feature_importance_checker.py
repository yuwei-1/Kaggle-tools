...
class RobustFeatureImportanceChecker:
    def __init__(
        ...
        initial_transform_list: List[callable] = [
    ...
    def _setup(self):
        ...
        settings = reduce(
            lambda acc, func: func(acc), self._initial_transform_list, settings
        )
        original_col_names = settings.training_col_names
        settings.update()
        combined_df, added_feature_names = self._feature_creator.create(
            settings.combined_df
        )
    ...
    def run(self):
        ...
        initial_score_population_path = os.path.join(
            self._result_path, "initial_score_population.npy"
        )
        csv_results_path = os.path.join(
            self._result_path, "robust_feature_importance.csv"
        )
        history = (
            pd.read_csv(csv_results_path)
        ...
        if os.path.exists(initial_score_population_path):
            ...
            for model_random_state in range(42, 42 + self._sqrt_population_size):
                for cv_random_state in range(42, 42 + self._sqrt_population_size):
                    ...
                    score_tuple, _, _ = CrossValidationExecutor(
                        model,
        ...
        for feature in self._added_feature_names:
            all_scores = []
            for model_random_state in range(
                1024, 1024 + int(self._sqrt_population_size * 0.5)
            ):
                for cv_random_state in range(
                    1024, 1024 + int(self._sqrt_population_size * 0.5)
                ):
                    model = self.get_model_instance(model_random_state)
                    self._kfold_object.random_state = cv_random_state
            ...
            print("RESULT: ", res)
            print(
                f"Original mean: {initial_score_population.mean()}, New mean: {feature_subsample.mean()}"
            )
            correct_direction = (
                (initial_score_population.mean() < feature_subsample.mean())
                if self._metric_direction == "maximize"
                else (initial_score_population.mean() > feature_subsample.mean())
            )
