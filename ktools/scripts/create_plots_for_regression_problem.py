import argparse
from pathlib import Path
import pandas as pd
from pandas.api.types import is_numeric_dtype, is_object_dtype
from ktools.visualization.regression import plot_professional_scatter
from ktools.visualization.classification import (
    plot_professional_violin,
    plot_professional_boxplot,
)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create plots for regression problem.")
    parser.add_argument("data_file", type=str, help="Path to the .csv data file.")
    parser.add_argument("target_column", type=str, help="Name of the target column.")
    parser.add_argument(
        "--output_dir",
        type=str,
        default="data/plots",
        help="Directory to save the plots.",
    )
    parser.add_argument(
        "--ignore_id",
        type=bool,
        default=True,
        help="Whether to ignore the 'id' column (default: True).",
    )
    args = parser.parse_args()

    data_file: str = args.data_file
    target_column: str = args.target_column
    output_dir = Path(args.output_dir)
    ignore_id: bool = args.ignore_id
    output_dir.mkdir(parents=True, exist_ok=True)

    data = pd.read_csv(data_file)
    target_values = data[target_column]
    features = [col for col in data.columns if col != target_column]

    for feature in features:
        if ignore_id and feature.lower() == "id":
            continue
        feat_values = data[feature]

        if is_numeric_dtype(feat_values):
            plot_professional_scatter(
                x=feat_values,
                y=target_values,
            ).write_image(output_dir / f"{feature}_scatter_plot.png")

        elif is_object_dtype(feat_values):
            plot_professional_violin(
                categories=feat_values,
                values=target_values,
            ).write_image(output_dir / f"{feature}_violin_plot.png")

            plot_professional_boxplot(
                categories=feat_values,
                values=target_values,
            ).write_image(output_dir / f"{feature}_boxplot.png")
        else:
            raise NotImplementedError(
                f"Feature type of column '{feature}' is not supported."
            )
