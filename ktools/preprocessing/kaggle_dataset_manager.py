...
class KaggleDatasetManager:
    ...
    def dataset_partition(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            self.dataframe[self._training_features],
            self.dataframe[self._target],
            test_size=self._test_size,
            random_state=self._random_state,
        )
        if self._validation_size:
            proportion_of_valid = self._validation_size / (
                self._validation_size + self._training_size
            )
    ...
    def _dataset_size_guard(*sizes):
        assert np.allclose(np.sum([*sizes]), 1, atol=0.01), (
            "dataset sizes must add to one"
        )
