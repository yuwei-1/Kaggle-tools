...
class FillInfValues(IFeatureTransformer):
    ...
    def transform(
        original_settings: DataSciencePipelineSettings,
        pos_inf_fill: float = -2,
        neg_inf_fill: float = -3,
    ):
        settings = deepcopy(original_settings)
        for col_name in settings.numerical_col_names:
            settings.combined_df.loc[
                settings.combined_df[col_name] == np.inf, col_name
            ] = pos_inf_fill
            settings.combined_df.loc[
                settings.combined_df[col_name] == -np.inf, col_name
            ] = neg_inf_fill
...
class FillNullMeanValues(IFeatureTransformer):
    @staticmethod
    def transform(
        original_settings: DataSciencePipelineSettings, category_fill="missing"
    ):
        settings = deepcopy(original_settings)
        for col_name in settings.training_col_names:
            if pd.api.types.is_numeric_dtype(settings.combined_df[col_name]):
                settings.combined_df[col_name] = settings.combined_df[col_name].fillna(
                    settings.combined_df[col_name].mean()
                )
            else:
                settings.combined_df[col_name] = settings.combined_df[col_name].fillna(
                    category_fill
    ...
    def transform(
        ...
        for col_name in settings.numerical_col_names:
            imputer = SimpleImputer(
                strategy=imputation_strategy, add_indicator=add_indicator
            )
            ...
            if train_transformed.shape[1] > 1:
                ...
                settings.combined_df.loc["train", [col_name, nan_col_name]] = (
                    train_transformed
                )
                settings.combined_df.loc["test", [col_name, nan_col_name]] = (
                    test_transformed
                )
...
class MinMaxScalerNumerical(IFeatureTransformer):
    ...
    def transform(original_settings: DataSciencePipelineSettings):
        ...
        train_df, test_df = settings.update()
        num_cols = (
            settings.combined_df[settings.training_col_names]
...
class RemoveTrainTargetOutliers(IFeatureTransformer):
    ...
    def transform(
        ...
        settings.combined_df = pd.concat(
            [no_outlier_train, test_df], keys=["train", "test"]
        )
        return settings
