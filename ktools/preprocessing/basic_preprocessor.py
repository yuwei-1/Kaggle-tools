...
class BasicPreprocessor(IPreprocessingUtility):
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame, DataSciencePipelineSettings]:
        ...
        train_data_col_names = kaggle_train_data.drop(columns=self._dss.target_col_name).columns

        if self._verbose:
            non_matching_col_names = set(test_data_col_names) ^ set(train_data_col_names)
            print(f"The following columns are not consistent across data sets: {non_matching_col_names}")
        
        self._dss.training_col_names = list(set(test_data_col_names).intersection(set(train_data_col_names)))
        self._dss.categorical_col_names = [col_name for col_name in self._dss.training_col_names if kaggle_train_data[col_name].dtype == 'object']

        kaggle_train_data = CategoricalLabelErrorImputator(verbose=self._verbose).impute(kaggle_train_data.copy(),
                                                                                         self._dss.categorical_col_names,
                                                                                         self._dss.category_occurrence_threshold)
        kaggle_test_data = CategoricalLabelErrorImputator(verbose=self._verbose).impute(kaggle_test_data.copy(),
                                                                                        self._dss.categorical_col_names,
                                                                                        self._dss.category_occurrence_threshold)
        kaggle_train_data, kaggle_test_data = self._process_categorical_columns(kaggle_train_data, 
                                                                                kaggle_test_data)
        if self._return_categorical:
            kaggle_train_data, kaggle_test_data = self._convert_to_categorical(kaggle_train_data), self._convert_to_categorical(kaggle_test_data)
    ...
    def _convert_to_categorical(self, dataframe):
        dataframe[self._dss.categorical_col_names] = dataframe[self._dss.categorical_col_names].astype("category")
        return dataframe