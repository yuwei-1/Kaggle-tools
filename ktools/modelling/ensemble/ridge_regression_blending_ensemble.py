...
class RidgeRegressionBlendingEnsemble(IEnsembleMethod):
    ...
    def fit_weights(self):
        ...
        metric_value = self._metric(
            self._labels, self._post_transform(blended_oof_preds)
        )
        ...
        if self._plot:
            ...
            display_scores = (
                pd.DataFrame(
                    {"predictor_name": self._oofs.columns, "score": coefficients}
                )
                .sort_values(by="score")
                .reset_index(drop=True)
            )
            display_scores = display_scores.style.background_gradient(
