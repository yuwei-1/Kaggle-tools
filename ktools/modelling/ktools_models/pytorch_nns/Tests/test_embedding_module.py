...
class TestEmbeddingModule(unittest.TestCase):
    def test_embedding_forward(self):
        # arrange
        X = torch.cat(
            [torch.randint(0, 6, (10, 1)), torch.randint(0, 4, (10, 1))], dim=-1
        )

        # act
        embedding_mod = EmbeddingCategoricalModule(
            category_cardinalities=[6, 4], embedding_sizes=[2, 2], projection_dim=5
        )
        y = embedding_mod(X)

        embedding_mod = EmbeddingCategoricalModule(
            category_cardinalities=[6, 4], embedding_sizes=[2, 2], projection_dim=None
        )
