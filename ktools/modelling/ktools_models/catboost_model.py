...
class CatBoostModel(BaseKtoolsModel, JoblibSaverMixin):
    ...
    def fit(
        ...
        train_pool = Pool(
            data=X_train, label=y_train, cat_features=self.cat_col_names, weight=weights
        )
        if self._early_stop:
            train_params["eval_set"] = Pool(
                data=X_valid, label=y_valid, cat_features=self.cat_col_names
            )
