...
from keras.api.layers import Input, Embedding, Flatten, Activation, Add, Dot
...
def predict_batch(model, X_t, batch_size=128):
    ...
    for X_batch, y_batch in test_batch_generator(
        X_t, np.zeros(X_t[0].shape[0]), batch_size=batch_size
    ):
...
def build_model(max_features, K=8, solver="adam", l2=0.0, l2_fm=0.0):
    ...
    for emb1, emb2 in itertools.combinations(flatten_layers, 2):
        dot_layer = Dot(axes=(1, 1))(
            [emb1, emb2]
        )  # merge([emb1,emb2],mode='dot',dot_axes=1)
    ...
    model.compile(optimizer=solver, loss="binary_crossentropy", metrics=["auc"])
