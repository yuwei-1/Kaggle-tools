...
class KNNModel(IKtoolsModel):
    def __init__(
        ...
        self._target_enc = TargetEncoder(
            target_type=target_type, smooth=smooth, random_state=random_state
        )
    ...
    def fit(self, X, y, **kwargs):
        present_categoricals_cols = list(
            set(X.columns).intersection(set(self._categorical_features))
        )
        target_enc_values = self._target_enc.fit_transform(
            X[present_categoricals_cols], y
        )
