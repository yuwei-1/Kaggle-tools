...
class KerasEmbeddingModel(IKtoolsModel):
    def __init__(
        ...
        if isinstance(categorical_embedding_sizes, int):
            self._categorical_embedding_sizes = [categorical_embedding_sizes] * len(
        ...
        if plot_model:
            tf.keras.utils.plot_model(model=self.model, show_shapes=True, rankdir="TB")

    def _build_model(self):
        cat_inputs = [
            layers.Input(shape=(1,), name=f"cat{i}")
            for i in self._categorical_feature_idcs
        ]
        ...
        for i in range(len(self._categorical_feature_idcs)):
            ...
            if self._categorical_embedding_sizes is not None:
                ...
                output_dim = int(min(128, round(1.6 * input_dim**0.56)))
            ...
            if output_dim > 32:
                embedding = layers.SpatialDropout1D(
                    min(0.5, self._dense_dropout + 0.2)
                )(embedding)
            else:
        ...
        x = layers.Dense(
            self._dense_hidden_sizes[0], activation=self._dense_activation
        )(concatenated_inputs_bn)
    ...
    def fit(self, X, y, validation_set=None, val_size=0.05):
        ...
        _ = self.model.fit(
            self._convert_to_nn_input(X_train),
            y_train,
    ...
    def predict(self, X):
        y_pred = self.model.predict(
            self._convert_to_nn_input(X), verbose=0, batch_size=self._batch_size
        ).flatten()
        return y_pred
