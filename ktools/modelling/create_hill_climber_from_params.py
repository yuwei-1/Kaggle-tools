...
class CreateHillClimber:
    ...
    def fit(self):
        ...
        X_test, y_test = self._test_df.drop(columns=self._target_col_name), self._test_df[self._target_col_name]
        test_oof_list = []
        train_oof_list = []
        ...
        for model_name, model in zip(self._model_names, self._model_list):
            ...
            _, train_oof, model_list = CrossValidationExecutor(model,
                                                                            self._eval_metric,
                                                                            self._kfold,
                                                                            ).run(_X_train, y)
        ...
        self.train_oof = train_oof = pd.concat([pd.Series(arr, index=X.index, name=name) for name, arr in train_oof_list], axis=1)
        self.test_preds = test_preds = pd.concat([pd.Series(arr, index=X_test.index, name=name) for name, arr in test_oof_list], axis=1)
        
        hill_climber = HillClimber(self._train_df,
                                    train_oof,
                                    test_preds,
