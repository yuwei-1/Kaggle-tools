...
from ktools.modelling.pytorch_utils.numpy_to_tensor_generator import (
    numpy_custom_torch_dataloader,
)
...
class VariationalGP(IKtoolsModel):
        ...
        if self._noise is None:
            self.likelihood = GaussianLikelihood(
                noise_constraint=gpytorch.constraints.GreaterThan(
                    self._default_noise_level / 10
                )
            )
            self.likelihood.noise_covar.initialize(noise=self._default_noise_level)
        else:
            self.likelihood = FixedNoiseGaussianLikelihood(
                noise=self._noise, learn_additional_noise=self._learn_additional_noise
            )
    ...
    def _find_clusters(self, X: pd.DataFrame, num_clusters: int) -> np.ndarray:
        ...
        kmeans.fit(X)
        idcs = (
            pd.DataFrame(data={"cluster": kmeans.labels_})
    ...
    def fit(
        ...
        if self._smart_inducing:
            ...
            X_inducing = X.sample(
                n=self._num_inducing_points, random_state=self._random_state
            )
        ...
        num_data = y.shape[0]
        inducing_points = torch.tensor(
            X_inducing.values, dtype=torch.float32, device=self._device
        )
        ...
        for epoch in epochs_iter:
            total_loss = 0
            train_loader = numpy_custom_torch_dataloader(
                X,
                y,
                batch_size=self._batch_size,
            ...
            try:
                training = epoch_progress.add_task(
                    f"[red]Training epoch {epoch}...",
                    total=round(num_data / self._batch_size),
                )
    ...
    def predict(self, X: ndarray, return_std: bool = False) -> torch.Tensor:
        ...
        test_loader = numpy_custom_torch_dataloader(
            X, batch_size=self._batch_size, shuffle=False, dtype=torch.float32
        )

        means = torch.tensor([0.0], device=self._device)
        # console = Console(force_terminal=True)
        eval_progress = Progress()
        eval_progress.start()

        try:
            evaluating = eval_progress.add_task(
                "[red]Evaluating...", total=round(num_data / self._batch_size)
            )
            with torch.no_grad():
