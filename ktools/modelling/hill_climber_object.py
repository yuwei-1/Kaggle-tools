...
class HillClimber:
    def __init__(
        ...
        model_prediction_file_paths: Dict[
            str, Tuple[str]
        ] = None,  # train oof, test file paths
    ...
    def naive_hill_climb(self):
        ...
        best_test_pred = climb_hill(
            train=self._train_df.iloc[available_oofs.index],
    ...
    def load_saved_prediction_files(self, model_prediction_file_paths):
        if model_prediction_file_paths is not None:
            for model_name, (
                train_oof_file_path,
                test_pred_file_path,
            ) in model_prediction_file_paths.items():
                train_oof = pd.read_csv(train_oof_file_path, index_col=0).reset_index(
                    drop=True
                )
                test_pred = pd.read_csv(test_pred_file_path, index_col=0).reset_index(
                    drop=True
                )
                ...
                test_pred.rename(
                    columns={
                        test_col_name[i]: train_col_name[i]
                ...
                train_oof.rename(
                    columns={
                        col_name: col_name + model_name for col_name in train_col_name
                    },
                    inplace=True,
                )
                test_pred.rename(
                ...
                self.train_oof_pred = pd.concat(
                    [self.train_oof_pred, train_oof], axis=1
                )
