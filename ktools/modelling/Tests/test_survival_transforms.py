...
class TestSurvivalTransform(unittest.TestCase):
    ...
    def test_transform_kaplan_meier(self):
        transform = transform_kaplan_meier(self.train_df['efs_time'], self.train_df['efs'])
    ...
    def test_transform_kaplan_meier_with_separation(self):
        transform = transform_kaplan_meier(self.train_df['efs_time'], self.train_df['efs'], separation=0.3)
        metric_value = self.scci_metric(self.train_df, transform)

        yes_event = self.train_df.efs.values == 1
        plt.figure()
        plt.hist(transform[yes_event], bins=100, color='orange', label="Patient died")
        plt.hist(transform[~yes_event], bins=100, color='blue', label="Patient death not observed")
        plt.xlim(0,1)
        plt.legend()
        plt.show()
        self.assertTrue(metric_value > 0.5)
    ...
    def test_transform_lognormal(self):
        transform = transform_log_normal(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_loglogistic(self):
        transform = transform_log_logistic(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_nelson_aalen(self):
        transform = transform_nelson_aalen(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_partial_hazard(self):
        transform = transform_partial_hazard(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_aalen_johansen(self):
        transform = transform_aalen_johansen(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_breslow_flemingharrington(self):
        transform = transform_breslow_flemingharrington(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_exponential(self):
        transform = transform_exponential(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
    ...
    def test_transform_generalized_gamma(self):
        transform = transform_generalized_gamma(self.train_df['efs_time'], self.train_df['efs'])
        metric_value = self.scci_metric(self.train_df, transform)
        print(metric_value)
        plt.figure()
        plt.hist(transform, bins=100)
