...
class TestKtoolsRegressionUsingDataset(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        class FE():
            ...
            def transform(original_settings : DataSciencePipelineSettings):
                ...
                settings.combined_df['Policy Start Date'] = pd.to_datetime(settings.combined_df['Policy Start Date'])
                settings.combined_df['year'] = settings.combined_df['Policy Start Date'].dt.year
                settings.combined_df['month'] = settings.combined_df['Policy Start Date'].dt.month
                settings.combined_df['Policy Start Date'] = settings.combined_df['Policy Start Date'].astype(str)
                settings.combined_df['cat_health_score'] = settings.combined_df['Health Score']
    ...
    @parameterized.expand([
        (XGBoostModel(**{"objective" : "reg:squarederror", "early_stopping_rounds" : None}), XGBRegressor(enable_categorical=True)),
        (CatBoostModel(**{"loss_function" : "RMSE",
                          "random_state" : None, 
                          "early_stopping_rounds" : None, 
                          "num_boost_round" : 1000}), CatBoostRegressor()),
        (LGBMModel(**{'objective': 'regression', "early_stopping_rounds" : None}), LGBMRegressor())
    ])
    def test_compare_regression_scores(self, model_cls : IKtoolsModel, original_model):
        if isinstance(original_model, CatBoostRegressor):
            original_model = CatBoostRegressor(cat_features=self.settings.categorical_col_names,
