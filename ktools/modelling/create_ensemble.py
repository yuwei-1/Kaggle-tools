...
class CreateFlatEnsemble:
    def __init__(
        ...
        self._model_cls_list, self._param_set_list, self._model_ids = (
            self._load_models()
        )
    ...
    def _fit_then_predict(self, X, y, X_test, model_idx: int):
        model = self._model_cls_list[model_idx](**self._param_set_list[model_idx]).fit(
            X, y
        )
    ...
    def create_model_predictions(self, X, y, X_test):
        ...
        for i, (train_index, val_index) in enumerate(self._kf.split(X, y)):
            ...
            for midx in range(self._num_models):
                y_pred, model = self._fit_then_predict(
                    X_train_fold, y_train_fold, X_val_fold, model_idx=midx
                )
        ...
        for midx in range(self._num_models):
            ...
            oof_prediction_df = pd.DataFrame(
                data=oof_predictions[midx], columns=["oof_predictions"], index=X.index
            )
            test_prediction_df = pd.DataFrame(
                data=test_predictions[midx],
...
if __name__ == "__main__":
    ...
    ensemble = CreateFlatEnsemble(
        "/Users/yuwei-1/Documents/projects/Kaggle-tools/ktools/modelling/Data/test_ensemble.json",
        kf,
        10,
    ...
    train_df = pd.read_csv(
        "/Users/yuwei-1/Documents/projects/Kaggle-tools/data/used_car_prices/basic_train.csv",
        index_col=0,
    )
    test_df = pd.read_csv(
        "/Users/yuwei-1/Documents/projects/Kaggle-tools/data/used_car_prices/basic_test.csv",
        index_col=0,
    )
    ...
    test_pred = ensemble.create_model_predictions(
