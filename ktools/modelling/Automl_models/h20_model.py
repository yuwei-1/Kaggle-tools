...
class KToolsH2OWrapper(IAutomlWrapper):
    ...
    def _set_model_name_and_save_paths(self, model_name):
        ...
        self._test_save_path = os.path.join(
            self._save_path, f"{model_name}_h2o_test.csv"
        )
    ...
    def fit(self):
        ...
        x = [
            ...
            if col not in [self._target_col_name, self.kfname]
        ]
        self.model.train(
            x=x,
            y=self._target_col_name,
            training_frame=h_train,
            fold_column=self.kfname,
        )
    ...
    def predict(self, df: Union[pd.DataFrame, None] = None):
        ...
        if df is not None:
            ...
            for model_id in self.model_ids:
                model = h2o.get_model(model_id)
                oof_predictions = (
                    model.cross_validation_holdout_predictions().as_data_frame()
                )
