...
def custom_torch_dataloader(*dataframes : pd.DataFrame, 
    ...
    fold_obj = KFold(n_splits=batches, shuffle=False) if not shuffle else KFold(n_splits=batches, shuffle=True, random_state=42)

    convert_batch_to_tensor = lambda df, idcs : torch.tensor(df.iloc[idcs].values, dtype=torch.float32)
