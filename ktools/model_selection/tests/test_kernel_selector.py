...
class TestKernelSelector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.configs = [
            ...
            GPConfig(
                mean_module=ConstantMean().initialize(constant=0.0),
                covariance_module=ScaleKernel(MaternKernel(nu=2.5, ard_num_dims=train_size)),
    ...
    def test_select(self):
        ...
        # Arrange
        X = pd.read_csv("./ktools/model_selection/tests/test_data/test_sample_data.csv", index_col=0)
        y = X.pop("label")
        subset_X = X.sample(n=1000)

        selector = VariantionalKernelSelector(self.configs,
