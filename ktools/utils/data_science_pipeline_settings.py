...
class DataSciencePipelineSettings(object):
    ...
    def __post_init__(self):
        ...
        (
            self.training_col_names,
            self.categorical_col_names,
            self.numerical_col_names,
        ) = self._get_column_info()
        self.combined_df = self._combine_datasets()

    def _gather_data(self):
        train_df = self._smart_drop_index(
            pd.read_csv(self.train_csv_path)
            if self.train_data is None
            else self.train_data
        )
        test_df = self._smart_drop_index(
    ...
    def get_data(self):
        self.update()
        X_test, y_test = (
            self.test_df.drop(columns=self.target_col_name),
            self.test_df[self.target_col_name],
        )
        X, y = (
